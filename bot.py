...
def safe_edit_message_text(call, new_text, new_markup, parse_mode="HTML"):
    ...
    try:
        ...
        if call.message.text != new_text:
            ...
            # Если текст тот же, просто отправляем уведомление
...
def get_all_users():
    """Получить список всех пользователей (только для владельца)"""
    conn = sqlite3.connect('atomy_bot.db')
    cursor = conn.cursor()
...
def get_bot_owner():
    """Определить владельца бота (первого пользователя в базе)"""
    conn = sqlite3.connect('atomy_bot.db')
    cursor = conn.cursor()
...
def get_referrer_info(referrer_id):
    ...
    # Если реферер не найден, возвращаем владельца бота
    return get_bot_owner()
...
def cmd_start(message):
    ...
    # Определяем, чей это реферал или владелец бота
...
Распространение премиум продукции повседневного спроса идет путем сетевого маркетинга через интернет-магазин компании."""
    ...
    bot.send_message(message.chat.id, company_text,
                     parse_mode="HTML",
                     reply_markup=get_company_keyboard())


# ========== РАЗДЕЛ "ПОЧЕМУ ИМЕННО ATOMY?" ==========
...
😍Поделиться знаниями и научить необходимым навыкам!"""
    ...
    # Клавиатура для раздела о бизнесе
...
⬇️⬇️⬇️⬇️⬇️"""
    ...
    btn5 = types.InlineKeyboardButton("🏠 Главное меню", callback_data="main_menu")  # Главное меню теперь btn5
    markup.add(btn1, btn2, btn3, btn4, btn5)  # Убрали btn5 (старый) и оставили 5 кнопок
...
Нажмите кнопку ниже чтобы вернуться назад:"""
...
Период подсчета: с 1 по 15 число месяца, с 16 по конец месяца."""
    ...
    # Клавиатура с 8 кнопками мастеров
    markup = types.InlineKeyboardMarkup(row_width=1)
...
def master_reward_callback(call):
    ...
    # Просто вызываем функцию квалификации мастеров
...
# ========== БОНУСЫ И ПООЩРЕНИЯ ДЛЯ МАСТЕРОВ ==========
@bot.callback_query_handler(func=lambda call: call.data == "master_reward")
def master_reward_callback(call):
    """Перенаправление на раздел квалификации мастеров"""
    master_qualification_callback(call)
...
def cmd_ref(message):
    """Генерация реферальной ссылки (только для владельца)"""
    user_id = message.from_user.id
    ...
    # Создаем реферальную ссылку для владельца
    bot_info = bot.get_me()
...
def cmd_users(message):
    """Показать список всех пользователей (только для владельца)"""
    user_id = message.from_user.id
    ...
    for i, (uid, username, full_name, ref_count, join_date) in enumerate(users[:50], 1):  # Ограничим 50 пользователей
...
def cmd_generate(message):
    """Генерация реферальных ссылок для других пользователей (только для владельца)"""
    user_id = message.from_user.id
    ...
    # Парсим команду: /generate 123456789
...
4️⃣мужская линия https://ch.atomy.com/ru/categories/65Подобрать уход за кожей лица прямо сейчас
...
def back_to_products_callback(call):
    # Возвращаемся к списку категорий продукции
    products_text = """Я с радостью расскажу о нашей продукции! 😉
...
def about_system(message):
    # Текст из раздела классификации
...
Посмотри какой рост может быть уже в первый месяц, два, три в твоем бизнесе!"""
    ...
    # Клавиатура с 5 уровнями + главное меню
    markup = types.InlineKeyboardMarkup(row_width=1)
    btn1 = types.InlineKeyboardButton("🛒 Торговый представитель", callback_data="sales_rep")
    btn2 = types.InlineKeyboardButton("🤝 Агент", callback_data="agent")
    btn3 = types.InlineKeyboardButton("⭐ Специальный агент", callback_data="special_agent")
...
def cmd_setowner(message):
    """Установить нового владельца бота (только если база пустая)"""
    user_id = message.from_user.id
    ...
    if count > 0:
        # Если в базе уже есть пользователи, проверяем текущего владельца
    ...
    # Обновляем информацию о пользователе как о владельце
    username = message.from_user.username
    full_name = message.from_user.first_name
    if message.from_user.last_name:
        full_name += " " + message.from_user.last_name
